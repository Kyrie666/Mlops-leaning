...
def train_and_infer(X_train_df, y_train, X_test_df, days, model_type):
    """
    Trains a model and performs inference using the specified model type.

    Args:
        X_train_df: The training data features as a DataFrame.
        y_train: The training data target values.
        X_test_df: The testing data features as a DataFrame.
        days: The number of days for which to perform inference.
        model_type: The type of model to use for training and inference.

    Returns:
        The predicted values for the testing data.
    """
    if model_type == "lgbm":
...
def predict_data(last_day):
    ...
    if add_prediction(result):
        logger.info("add predict data success!")
    else:
        logger.info("add predict data failed!")
    conn.close()
    return


def synchronize_data(today):
    """
    ...
    for department in ["bs", "jm", "sz", "gx"]:
        ...
        if add_new_time_data(new_data):
            logger.info(f"{department} add new data success!")
        else:
            logger.info(f"{department} add new data failed!")
            logger.info("synchronize data failed!")
            conn.close()
            return
